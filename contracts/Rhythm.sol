 ...
 * `onlyOwnerTest`, which can be applied to your functions to restrict their use to
 * the owner.
 */
...
contract RhythmOwnable is Initializable, Context {
     ...
     */
    function renounceOwnership() public virtual onlyOwnerTest() {
     ...
     */
    function transferOwnership(address newOwner) public virtual onlyOwnerTest() {
     ...
     * Can only be called by the current owner.
     */
    function setArtistWalletAddress(address newArtistWalletAddress) public virtual onlyOwnerTest() {
...
contract Rhythm is Initializable, Context, BEP20, RhythmOwnable, ReentrancyGuard {
    ...
    function initialize() public payable initializer {
        ...
        emit PancakeSwapRouterUpdate(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        isExchangeAddress[0x10ED43C718714eb63d5aA57B78B54704E256024E] = true;
    ...
    function blacklistAddress(address account) public onlyOwnerTest() {
        _isBlacklisted[account] = true;
    }

    function unBlacklistAddress(address account) public onlyOwnerTest() {
    ...
    // PancakeSwap / UniSwap / Exchanges, etc
    function addExchangeAddress(address account) public onlyOwnerTest() {
        isExchangeAddress[account] = true;
    }

    function removeExchangeAddress(address account) public onlyOwnerTest() {
    ...
    function excludeFromReward(address account) public onlyOwnerTest() {
    ...
    function includeInReward(address account) external onlyOwnerTest() {
    ...
    function excludeFromFee(address account) public onlyOwnerTest() {
        _isExcludedFromFee[account] = true;
    }
    
    // Remove an address from fee exclusion.
    function includeInFee(address account) public onlyOwnerTest() {
        _isExcludedFromFee[account] = false;
    }
    
    // Set reflection tax fee percentage.
    function setReflectionTaxFeePercent(uint256 reflectionTaxFee) external onlyOwnerTest() {
        _reflectionTaxFee = reflectionTaxFee;
        emit ReflectionTaxUpdate(reflectionTaxFee);
    }
    
    // Set the liquidity fee percentage
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwnerTest() {
        _liquidityFee = liquidityFee;
        emit LiquidityTaxUpdate(liquidityFee);
    }
    
    // Set the artist wallet fee percentage
    function setArtistFeePercent(uint256 artistFee) external onlyOwnerTest() {
        _artistFee = artistFee;
        emit ArtistTaxUpdate(artistFee);
    }
    
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwnerTest() {
        _maxTxAmount = maxTxAmount * 10**9;
        emit MaxTaxUpdate(maxTxAmount);
    }

    // Set minimum tokens required in contract to sell for liquidity
    function setNumTokensSellToAddToLiquidity(uint256 amount) external onlyOwnerTest() {
        numTokensSellToAddToLiquidity = amount.div(10**2);
        emit TokenSellToLiquidityUpdate(amount);
    }

    // Set liquidity swap
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwnerTest() {
        swapAndLiquifyEnabled = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }
    
    // Set the PancakeSwap router address.
    function setRouterAddress(address newRouter) public onlyOwnerTest() {
    ...
    */
    ...
    function resetUserDailySoldTokens(address user) public onlyOwnerTest() {
    ...
    function setMaxTokensSellPerDay(uint256 maxSellAmountPerDay) public onlyOwnerTest() {
        _maxSellAmountPerDay = maxSellAmountPerDay * 10**9;
    }

    function setSellLockTime(uint256 timeInHours) public onlyOwnerTest() {
        uint256 totalHours = timeInHours * 1 hours;
        sellLockTime = totalHours;
    }
    
    // Enable / Disable sell lock
    function enableOrDisableSellLock(bool newValue) public onlyOwnerTest() {
        sellLockEnabled = newValue;
    }

    // Enable / Disable trading
    function enableOrDisableTrading(bool newValue) public onlyOwnerTest() {
    ...
    function setUserTokensSoldInDay(address account, uint256 amount) public onlyOwnerTest() {
    ...
    */
    function withdrawLeftoverBNBToArtist(address payable recipient, uint256 amount) public onlyOwnerTest() {
        if (amount <= 0)
    ...
    function withdrawOtherTokens(BEP20 tokenAddress, address walletaddress) public onlyOwnerTest() {
